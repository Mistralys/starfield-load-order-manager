...
- Application supports multiple languages via .NET resource files (`.resx`).
- `LocalizationService` initialized in `App.OnStartup()` before any windows are created.
- Culture set from `AppConfigModel.PreferredLanguage` (defaults to `"auto"` for system culture detection).
- Culture changes applied to both `CurrentUICulture` and `CurrentCulture` for consistent formatting.
- Resource files must use `PublicResXFileCodeGenerator` as custom tool to generate public strongly-typed accessors.
- Designer files (`.Designer.cs`) auto-generated by Visual Studio; must regenerate after editing `.resx` files via "Run Custom Tool".
- All user-facing strings should be stored in resource files, not hardcoded in code or XAML.
- ViewModels expose localized strings as properties that subscribe to `LocalizationService.CultureChanged` event for dynamic updates.
- `UserMessages` class serves as facade over `CommonResources` for centralized message access across the application.
- Satellite assemblies (`.resources.dll`) automatically generated in culture-specific folders (e.g., `bin/Debug/net9.0-windows/fr/`).
- Culture files named with culture suffix (e.g., `CommonResources.fr.resx` for French, `CommonResources.de.resx` for German).
- `FlowDirection` property prepared in views for future RTL (Right-to-Left) language support.
