...
- `LocalizationService` singleton (in `ViewTexts/`) accessed via `LocalizationService.Instance`; initialized in `App.OnStartup()` before any windows via `LocalizationService.InitializeFromConfig(preferredLanguage)`.
- Culture preference loaded from `AppConfigModel.PreferredLanguage` (defaults to `"auto"` for system culture detection).
- `LocalizationService.SetCulture(cultureName)` loads the corresponding JSON locale file from `ViewTexts/Locales/` and applies the culture to `CurrentUICulture` and `CurrentCulture`.
- Available locales discovered dynamically by scanning `ViewTexts/Locales/*.json`; display names read from `LocaleName` field in each file.
- Parent culture mapping (e.g., `fr-CA` → `fr-FR`) read from `ParentCulture` field in each JSON file, enabling automatic system locale detection without hardcoding.
- Text ViewModels (e.g., `MainWindowTexts`, `SettingsWindowTexts`) expose localized strings to the UI by calling `LocalizationService.Instance.GetString(section, key)`.
- `LocalizationService.CultureChanged` event notifies Text ViewModels to refresh their observable properties when the culture changes.
- Language dropdown in Settings populated via `SettingsViewModel.AvailableLanguages`, built by calling `LocalizationService.GetAvailableCultures()` and `GetLocaleName()`.
- `LocalizationJsonNormalizer` is a development-only utility (not used at runtime) for normalizing JSON files before committing to version control.
...
- On differences, `FileService.WouldSortingChangeDiffsAsync()` sets the sorting recommendation, `DiffService.GetPluginsDiffAsync()` feeds both the badge count and the `DiffDialogViewModel`. Internally, `GetPluginsDiffAsync` runs an LCS-based classification pipeline: it reads both the reference and current mod lists via `FileService.ReadModListAsync`, computes their Longest Common Subsequence (the "stable spine" of mods that maintained relative order) via `DiffService.ComputeLcs`, then classifies all changes via `DiffService.ClassifyChanges` — producing `Moved`, `Replaced`, `Inserted`, `Added`, and `Removed` entries, with position-shifted LCS items grouped as `DependentChanges` under their causal insertion or deletion.
